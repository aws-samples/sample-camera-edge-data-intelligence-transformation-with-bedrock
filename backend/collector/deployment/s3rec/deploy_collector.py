#!/usr/bin/env python3
"""
S3Rec CloudFormationデプロイスクリプト
deploy_s3rec.shの143行目以降をPython化したもの
...
def deploy_cloudformation_stack(camera_id, collector_id, source_s3_bucket):
    ...
    print("==================================================")
    print("  Cedix CloudFormation S3Rec デプロイ       ")
    print("==================================================")
    print()
    ...
    stack_name = get_collector_stack_name(camera_id, "s3Rec", collector_id)
    if not stack_name:
    ...
    parameter_mapping = {
        ...
        'S3RecRepositoryUri': '/Cedix/Ecr/S3RecRepositoryUri',
    ...
    ecr_repository_uri = parameter_values['S3RecRepositoryUri']
    ...
    for param_name, value in parameter_values.items():
        if param_name != 'S3RecRepositoryUri':  # ECRリポジトリURIは別途表示
    ...
    cf_parameters = [
        ...
        {'ParameterKey': 'S3RecRepositoryUri', 'ParameterValue': repository_uri_with_tag},
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='S3Rec CloudFormationデプロイスクリプト')
    ...
    while True:
        ...
        if status == 'SUCCESS':
            print()
            print("🎉 S3Rec Lambda関数のデプロイが正常に完了しました！")
            print(f"スタック名: {stack_name}")
