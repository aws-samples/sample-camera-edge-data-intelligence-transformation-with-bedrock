...
| **AIMLによる映像解析** | 集約した映像解析には、YOLOv9(MIT) によるリアルタイム解析、Amazon Bedrockによる画像/映像解析を利用可能。解析結果として画像/動画に任意のタグ付けがされる。<br>別途独自のモデルも組み込み可能。 |
| **24365の自動映像確認** | 人間が24時間分の映像を全て見る必要なし。AIが検出した重要イベントは通知可能。また通知結果を１クリックで確認可能 |
...
リアルタイムについては、監視カメラ(RTSP/RTMP)やVSaaS カメラ を代表とする映像をストリーミングする媒体から映像を取得しHLSエンドポイントを提供します。基本的には映像は最終的にKinesis Video Streamに変換され、そのHLSエンドポイントが後続で利用されます。VSaaSのようにデフォルトでHLSエンドポイントを生成する機能がある場合、後続は直接そのエンドポイントを利用します (なお、別途 CameraManagementを開発することで、VSaaS映像をKinesis Video Streamに集約化することも可能)
また、バッチについては、動画や画像を直接アップロードする形での映像収集の機能となります。CEDIXはファイル収集用のS３バケットを提供するため、そこにアップロードしたものはコレクターを経由して時系列に管理されます
...
- RTSPカメラ
  ...
  - 1つは AWS 側からRTSPカメラに対して接続するパターン。その場合はRTSPカメラを配置しているネットワークでポート開放が必須となります。なおこのテストのために「テスト動画」機能があります。
  - もう１つは RTSPカメラのネットワークに、RTSP収集クライアントを配置するパターン。CEDIXはRTSP収集クライアントを Dockerfileで用意しています。それを任意の端末でセットアップして動作させることで映像収集可能です。
- RTMPカメラ
  - CEDIXでは、RTMPエンドポイントを公開し、RTMPクライアント機能搭載のカメラからの映像を受信することが出来ます。テストでしたら、スマートフォンのRTMPクライアント機能のついたアプリなどからもテスト可能です。
...
- それ以外の映像ソースについて
  ...
  - そのため、カメラ搭載システムなどで、KVSに映像を投げることが出来れば、CEDIXに映像を連携可能です

#### バッチ収集
- S3
  - CEDIX では S3 タイプのカメラを作成すると、S3の収集エンドポイント(S3 バケットのパス)を生成します
  - そのS3パスに画像/動画を送信すれば、バックエンドのS3コレクターが時系列に保存/管理してくれます。また、そのタイミングにて YOLOv9(MIT)での解析や、Amazon Bedrockによる解析も実施可能です。
...
また、収集している映像ソースに対してリアルタイムにYOLOv9(MIT)モデルで画像解析する機能もあります。具体的に、リアルタイムでの物体検出および、指定エリアの領域侵入/退出判定を行う機能も用意しています。
...
- 用途については、そのままご利用頂いても良いですし、ご自身のモデルをCEDIXに組み込む際の参考として使って頂くのも良いです。また、Amazon Bedrockによる検出回数を減らしてコスト最適化するためのフィルター機能としてご利用頂くことは推奨しています (画像/動画が作成されるたびに常に 生成AIを用いて解析すると若干コストがかかるため)
...
#### 拡張するためのアイデア
- Collectorは、直接映像ソースに接続をして処理をするモジュールのため、YOLO検出機能のようにリアルタイムで処理を行いたい場合は、このCollectorを拡張するのが推奨されます。
- 例えば
  - リアルタイムに、自社独自の画像解析モデルでの分析を多ない、その検出結果を保存＆通知させる。

### Detector
![1770450942980.png](doc/image/1770450942980.png)
Amazon Bedrockの生成AIモデルを利用して、映像解析およびタグ付けを行います。映像から検出したいタグについて自然言語で指定可能です。カメラ毎に自然言語で解析したい内容を指定できるため、現場でAIの知識がない人でもカメラの設定が可能です。

#### タグ作成&管理機能
- Bedrock に検出させたいタグは、事前にタグ管理機能にて登録しておく必要があります。タグ管理画面を開き、タググループ、タグの順番で登録が必要です。
- カメラごとに１からタグを登録することも可能ですが、多くの場合、同じタグを複数カメラに設定する必要がでます。そのため、基本的にタグ管理を上手く利用することが推奨です。
- なお、Detectorの登録画面ではタググループの一括ロードまたは、個別タグの個別ロードが可能になっていますので、柔軟に利用できます。
...
- 設定としては、「基盤モデル」指定した上で、以降は自然言語にて「役割」の指定、「検出したいもの」の指定、「検出したものがどういう状態であれば、どういうタグを出力するか」のタグ設定 を行います。タグ設定は前述の通り、簡単にロード＆設定可能です。
...
- 例えば
  - 社内の様々な画像解析APIを Amazon Bedrock AgentCore GatewayにてMCP化しつつ、それと接続する 独自のAIエージェントをAmazon Bedrock AgentCore Runtime で稼働させ、それを Lambda経由で駆動させることで、複雑な画像分析処理も可能になります。
  - Amazon Rekognition の顔検出&分析機能を利用することで、人が映った写真から誰が映っているか？を具体的に判断し、タグとして保存をすることで、いつ、どのカメラに、誰が映ったかを分析可能なシステムが実現します。


### Notify
- Detectorでタグを登録する際に、Notify をONにした場合、そのタグが検出された時点で通知が実施されます。現状は、あくまでCEDIXのUI内だけでの通知表示となります。
...
### Analytics
検出された結果は、cedix-detect-log テーブル に格納されます。と同時に、DynamoDB Stream+Lambdaを経由して Amazon OpenSearch Service 連携されます。分析は一部を除き、OpenSearchに格納されたデータを利用して実施されます。
...
- ただし、この機能は本当の意味でサンプルです。本来は指定した画像/動画をバイナリとして読み込んで指定に基づいて解析した方がより正確ですが、現在はDetectorの検出結果をベースに解析する簡便な分析を実施します。
...
```
- 一括デプロイで失敗する場合、個別デプロイを推奨します。詳細は [CDK_ARCHITECTURE](_doc/CDK_ARCHITECTURE.md) を参照ください。なお、一括デプロイの前に`./run-cdk.sh deploy cedix-<prefix>-kvs-base-ecr`  だけは先行でデプロイをした方が失敗が少ないので推奨です。
